...
<div class="main">
    <div class="topbar">
        ...
        <div class="tb-r">
            <div class="tb-date">{{ date('d M Y') }}</div>
            <div class="av">PO</div>
        </div>
    </div>
    ...
    <div class="content">
        ...
        <div class="stats">
            <div class="stat navy">
                ...
                <div class="stat-n">{{ count($staff) }}</div>
                <div class="stat-s">All hospital staff</div>
            </div>
            <div class="stat teal">
                <div class="stat-l">Permanent</div>
                <div class="stat-n">{{ $staff->where('contract_type','Permanent')->count() }}</div>
                <div class="stat-s">Permanent contracts</div>
            </div>
            <div class="stat">
                <div class="stat-l">Temporary</div>
                <div class="stat-n">{{ $staff->where('contract_type','Temporary')->count() }}</div>
                <div class="stat-s">Temporary contracts</div>
            </div>
            <div class="stat">
                <div class="stat-l">Positions</div>
                <div class="stat-n">{{ $staff->pluck('position')->unique()->count() }}</div>
        ...
        <div class="ph">
            ...
            <a href="{{ route('staff.create') }}" class="btn b-teal">+ New Staff</a>
        </div>
        ...
        <div class="card">
            <div class="card-hd">
                <div>
                    ...
                    <div class="card-sub">Showing {{ count($staff) }} staff members</div>
                </div>
            </div>
            <table class="tbl">
                <thead>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <div class="p-info">
                                <div class="p-av">{{ strtoupper(substr($s->first_name,0,1).substr($s->last_name,0,1)) }}</div>
                                <strong>{{ $s->first_name }} {{ $s->last_name }}</strong>
                            </div>
                        </td>
                        <td class="mono">{{ $s->staff_number }}</td>
                        <td><span class="bdg br">{{ $s->position }}</span></td>
                        <td class="mono">{{ $s->nin }}</td>
                        <td>£{{ number_format($s->current_salary, 2) }}</td>
                        <td>
                            <span class="bdg {{ $s->contract_type == 'Permanent' ? 'bp' : 'bt' }}">
                                {{ $s->contract_type }}
                            </span>
                        </td>
                        <td>
                            <div class="td-act">
                                <a href="{{ route('staff.show', $s->staff_number) }}" class="btn b-ol b-sm">View →</a>
                                <a href="{{ route('staff.edit', $s->staff_number) }}" class="btn b-ol b-sm">Edit →</a>
                                <form action="{{ route('staff.destroy', $s->staff_number) }}" method="POST" style="display:inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="7" style="text-align:center;padding:40px;color:var(--muted)">
                            No staff members found. <a href="{{ route('staff.create') }}" style="color:var(--teal)">Add one now →</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
